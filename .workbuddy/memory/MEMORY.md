...
### 6. Netlify Identity 邮件确认链接跳转首页问题（2026-04-06）
**问题**: 邮件中的确认链接（`/#confirmation_token=xxx`）点击后跳转到网站首页而非 `/admin/`
**原因**: 邮件链接未配置正确的回调 URL
**解决方案**: 启用 Netlify Identity GitHub 登录，绕过邮件确认流程
**状态**: ✅ 已成功配置并测试，可使用 GitHub 账号登录 CMS 后台
...
- 构建命令: `npm run build`
- 发布目录: `dist`
- **Identity**: 已启用 GitHub 登录，访问 `/admin/` 可通过 GitHub 授权登录
...
**最后更新**: 2026-04-06
**状态**: ✅ CMS 后台已可通过 GitHub 登录，可正常管理内容
